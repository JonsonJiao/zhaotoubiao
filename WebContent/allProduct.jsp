...
<div class="product">
	...
	<div class="productInclude">
		<div class="head">
			<ul >
			<li class="title1">招标名称</li>
			<li class="title2">发布人</li>
			<li class="title3">发布日期</li>
			</ul>
		</div>
		<% 
			int index=1;
		...
		<div class="main">
			<div class="title">
				<span><a href="product.jsp?listIndex=<%=zhaoBiao.getId()%>"><%= 
						zhaoBiao.getTitle()%></a></span>
				<span>单位名称：<%=zhaoBiao.getCompanyName() %></span>
			</div>
			<div class="price">
				<span><%=zhaoBiao.getUserName() %></span>
			</div>
			<div class="remaining">
		...
		%>
			<a href="allProduct.jsp?index=<%=index-1 %>">上一页</a>
		<%
			}
				int count =ZhaoBiaoFactory.get().countOfAll();
				count =count/ZhaoBiaoProxy.SIZE+1;
				int i =0;
				if((count-10)<0){
					i=0;
				}else{
				...
				for(int x=i;x<count;x++){
					if(x!=index-1){
		%>
			<a href="allProduct.jsp?index=<%=x %>"><%=x+1 %></a>	
		<% 		
				}else{
		%>
				<%=x+1 %>
		...
		%>
			<a href="allProduct.jsp?index=<%=index+1 %>">下一页</a>
		<%
			}
		%>
		</div>
